...
if type "xrandr"; then
  for m in $(xrandr --query | grep " connected" | cut -d" " -f1); do
    ...
    then
      ...
      MONITOR=$m polybar -q -c "$DIR"/config.ini top_right_monitor &
      MONITOR=$m polybar -q -c "$DIR"/config.ini bottom_right_monitor &

    elif [ $m == $left_monitor ]
    then
      echo "Starting top + bottom polybar for left monitor ($left_monitor)"
      MONITOR=$m polybar -q -c "$DIR"/config.ini top_left_monitor &
      MONITOR=$m polybar -q -c "$DIR"/config.ini bottom_left_monitor &
    ...
    # In the case where no monitor matches the setup, use current monitor m as single monitor and break
      ...
      MONITOR=$m polybar -q -c "$DIR"/config.ini top &
      MONITOR=$m polybar -q -c "$DIR"/config.ini bottom &
      break
    fi
  done
else
